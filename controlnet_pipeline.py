...
from cogvideo_controlnet import CogVideoXControlnet as CogVideoXControlnet
...
class ControlnetCogVideoXImageToVideoPCDPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    ...
    def __init__(
        ...
        self.register_modules(
            tokenizer=tokenizer, text_encoder=text_encoder, vae=vae, transformer=transformer, scheduler=scheduler, controlnet_features=controlnet_features
        )
    ...
    def __call__(
        ...
        if anchor_latents is not None:
            # Pre-encoded latents (e.g. depth) — skip VAE encoding
            anchor_states = anchor_latents.to(device, dtype=self.transformer.dtype)
        else:
        ...
        if do_classifier_free_guidance:
            ...
            if features is not None:
                features = torch.cat([features[None]] * 2).to(device, dtype=anchor_states.dtype)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                controlnet_states_features = None
                input_controlnet_states = anchor_states
                if (controlnet_guidance_start <= current_sampling_percent <= controlnet_guidance_end):
                    if self.controlnet_features is not None:
                        controlnet_states_features = self.controlnet_features(
                            hidden_states=latent_model_input,
                            encoder_hidden_states=prompt_embeds,
                            image_rotary_emb=image_rotary_emb,
                            controlnet_states=features,
                            controlnet_output_mask=None,
                            timestep=timestep,
                            return_dict=False,
                        )[0]
                        if isinstance(controlnet_states_features, (tuple, list)):
                            controlnet_states_features = [x.to(dtype=self.transformer.dtype) for x in controlnet_states_features]
                        else:
                            controlnet_states_features = controlnet_states_features.to(dtype=self.transformer.dtype)
                ...
                noise_pred = self.transformer(
                    ...
                    controlnet_states2=controlnet_states_features,
                    controlnet_weights=controlnet_weights,
                    spade_conditioning=getattr(self, 'spade_conditioning', False),
                    return_dict=False,
                )[0]
